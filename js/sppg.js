...
const TEMPLATES={
  ...
  'sppg-2':{name:'SPPG MBG BESUKI',info:'Template SPPG MBG BESUKI. Desain label mengikuti template MBG BESUKI; elemen tetap tidak perlu diisi manual.',count:15,showGizi:true},
  'sppg-3':{name:'SPPG MBG BESUKI BATCH 2',info:'Template SPPG MBG BESUKI Batch 2, ukuran 75 × 50 mm. Cukup isi logo, nama instansi, teks batch, dan jam batas konsumsi.',count:0,showGizi:false}
};
let state={instansi:'',alamat:'',menu:'',batasTime:'12:30',alertLeft:'TIDAK UNTUK DIBAWA PULANG',alertRight:'PORSI BESAR',logoData:'',edgeData:'',gizi:Array(15).fill(''),template:'etiket-lama',batch1Time:'09:00',batch2Time:'12:00',s3Batch:'Batch 2',s3Time:'12:00',printQueue:[]};
...
function buildGizi(){const grid=$('sppg-gizi-grid');grid.innerHTML='';const s2=($('template-select')?.value==='sppg-2');const nutrients=['Energi (kkal)','Protein (g)','Lemak (g)','Karbohidrat (g)','Serat (g)'];if(s2){grid.className='sppg-nutrition-input-grid s2-form-grid';for(let r=0;r<5;r++){const a=document.createElement('div');a.className='s2-form-row';const i1=document.createElement('input');i1.className='control';i1.id='gizi-'+(r*3);i1.placeholder='Menu '+(r+1);i1.value=state.gizi[r*3]||'';const i2=document.createElement('input');i2.className='control';i2.id='gizi-'+(r*3+1);i2.placeholder='Porsi Kecil';i2.value=state.gizi[r*3+1]||'';const i3=document.createElement('input');i3.className='control';i3.id='gizi-'+(r*3+2);i3.placeholder='Porsi Besar';i3.value=state.gizi[r*3+2]||'';[i1,i2,i3].forEach(x=>x.addEventListener('input',updatePreview));a.append(i1,i2,i3);grid.appendChild(a)}const note=document.createElement('small');note.textContent='Nama kandungan gizi (Energi, Protein, Lemak, Karbohidrat, Serat) mengikuti desain dan tidak perlu diisi manual.';grid.appendChild(note)}else{grid.className='sppg-nutrition-input-grid';for(let i=0;i<15;i++){const input=document.createElement('input');input.className='control';input.id='gizi-'+i;input.placeholder=i%3===0?'Nama':i%3===1?'Nilai':'Satuan';input.value=state.gizi[i]||'';input.addEventListener('input',updatePreview);grid.appendChild(input)}}}
function readForm(){state.instansi=$('sppg-instansi').value;state.alamat=$('sppg-alamat').value;state.menu=$('sppg-menu').value;state.batasTime=$('sppg-batas-time').value||'12:30';state.alertLeft=$('sppg-alert-left').value;state.alertRight=$('sppg-alert-right').value;state.template=$('template-select').value;state.batch1Time=$('sppg-batch1-time')?.value||state.batch1Time||'09:00';state.batch2Time=$('sppg-batch2-time')?.value||state.batch2Time||'12:00';state.s3Batch=$('sppg-s3-batch')?$('sppg-s3-batch').value:state.s3Batch;state.s3Time=$('sppg-s3-time')?.value||state.s3Time||'12:00';state.gizi=[...Array(15)].map((_,i)=>$('gizi-'+i)?.value||'')}
function renderLogo(){
  ...
  }else if($('sppg-logo-form-preview')){
...
function openLogoModal(){if(!state.logoData){$('sppg-logo-input').click();return}$('logo-modal-image').src=state.logoData;$('logo-modal').classList.add('show');$('logo-modal').setAttribute('aria-hidden','false')}
...
function applyTemplate(){
  ...
  $('s2-batch-fields').style.display=isS2?'block':'none';
...
function updatePreview(){
  ...
  document.querySelectorAll('.old-template-field').forEach(el=>el.style.display=(isS2||isS3)?'none':'block');
  if($('s2-batch-fields')) $('s2-batch-fields').style.display=isS2?'block':'none';
  if($('s3-fields')) $('s3-fields').style.display=isS3?'block':'none';
...
function renderSppg3(){
  // SPPG MBG BESUKI BATCH 2: satu sumber data dari Form Label SPPG -> Preview -> Cetak.
  const instansi=state.instansi||'NAMA INSTANSI';
  const batchText=(state.s3Batch===''?'':(state.s3Batch||'Batch 2'));
...
function renderSppg2(){
  // SPPG MBG BESUKI: satu sumber data dari Form Label SPPG -> Preview Pengaturan Print -> Cetak.
  ...
  let rows='';
  for(let r=0;r<5;r++){
    const menu=state.gizi[r*3]||'';
    const small=state.gizi[r*3+1]||'';
    const large=state.gizi[r*3+2]||'';
    rows+=`<tr><td>${esc(menu)}</td><td>${nutrient[r]}</td><td>${esc(small)}</td><td>${esc(large)}</td></tr>`;
  }
  ...
  ['preview-sppg2-batch1','print-sppg2-batch1'].forEach(id=>{
    const el=$(id);if(el)el.textContent=formatBatch(state.batch1Time||'09:00');
  });
  ['preview-sppg2-batch2','print-sppg2-batch2'].forEach(id=>{
    const el=$(id);if(el)el.textContent=formatBatch(state.batch2Time||'12:00');
  });

  // Logo dipasang ke dua tujuan SPPG MBG BESUKI.
